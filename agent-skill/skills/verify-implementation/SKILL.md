...
| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-frontend-conventions` | 파일 구조·명명·컴포넌트 규칙 검증 (barrel export, CSS+TSX 디렉토리, window.confirm, PascalCase, co-location) |
| 2 | `verify-client-directive` | 'use client' 남용 탐지 — 루트 layout 보호, hook 없는 'use client' 탐지 |
| 3 | `verify-api-contract` | API 레이어 규칙 검증 — lib/api↔types/api 대칭, apiFetch 전용, /api/ URL 패턴, 인라인 타입 금지 |
